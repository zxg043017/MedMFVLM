...
class ITRDataset(Dataset):
    def __init__(self, args, tokenizer, category_arr, mode="train"):
        self.args = args
        self.data_root = args.data_root
        self.tokenizer = tokenizer
        self.mode = mode
        self.windowprocess = False
        ...
        train_transform = mtf.Compose(
            [
                ...
                mtf.Resize(spatial_size=(96,96,96), mode="area"),
                # transforms.NormalizeIntensity(), # added by MR on 2024 0905
    ...
    def get_category(self, name):
        ...
        if name not in self.category_arr:
            name = name.split('_')[0]

        category = self.category_arr[name]
        return category
    
    def pretransform(self, image, name):
        category = self.get_category(name)
        
        if self.windowprocess:
            if category == 'head':
                pre_transform = mtf.Compose(
                    [
                ...
                pre_transform = mtf.Compose(
                    [
                        mtf.ScaleIntensityRanged(
                            keys=["image"], a_min=-1400, a_max=200,
                            b_min=0.0, b_max=1.0, clip=True,
                        ),
                ...
                pre_transform = mtf.Compose(
                    [
                        mtf.ScaleIntensityRanged(
                            keys=["image"], a_min=-500, a_max=1500,
                            b_min=0.0, b_max=1.0, clip=True,
                        ),
                        # transforms.ToTensord(keys=["image"]),
                    ]
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                image_path = data["image"] # image address
                path_1 = image_path.split("_")[0]+'_'+image_path.split("_")[1]
                path_2 = image_path.split("_")[0]+'_'+image_path.split("_")[1]+image_path.split("_")[2]
                image_abs_path = os.path.join(self.data_root,path_1,path_2, image_path)
                #image_abs_path = os.path.join(self.data_root, image_path)
                # cd ("zxg:image_abs_path",image_abs_path)
                name = image_path.split('.')[0]
                
                if self.windowprocess:
                    image = np.load(image_abs_path)
                    image = self.pretransform(image, name)
                    image = self.transform(image['arr_0'])
                else:
                    image = np.load(image_abs_path)['arr_0']  # nomalized 0-1, C,D,H,W
                    image = self.transform(image)
                    
                # image = np.load(img_abs_path)[np.newaxis, ...]  # nomalized
                #print("zxg:image.shape",image.shape) #1*32*256*256, normalized
                raw_text = data["caption"]
                # text_path = data["text"] # text address
