...
def parse_args(args=None):
    ...
    parser.add_argument('--model_name_or_path', type=str, default="./LaMed/output/LaMed-Phi3.5-mini-finetune-0128/checkpoint-370000", choices=[])
    ...
    # data
    parser.add_argument('--data_root', type=str, default="/raid/export/wqruan/m3d_tao")
    parser.add_argument('--cap_data_path', type=str, default="/raid/export/wqruan/m3d_tao/tao_train_clip.json")
    parser.add_argument('--output_dir', type=str, default="./LaMed/output/LaMed-finetune-0129/eval_caption/")
...
def main():
    ...
    with open(output_path, mode='w') as outfile:
        ...
        rouge_mean = np.mean(rouges)
        meteor_mean = np.mean(meteors)
        bert_mean = np.mean(bertf1)
        bleu_std = np.std(bleus)
        rouge_std = np.std(rouges)
        meteor_std = np.std(meteors)
        bert_std = np.std(bertf1)
        json_data = {"bleu_mean": bleu_mean, "rouge_mean": rouge_mean, "meteor_mean": meteor_mean, "bert_mean": bert_mean,
                     "bleu_std": bleu_std, "rouge_std": rouge_std, "meteor_std": meteor_std, "bert_std": bert_std}
