...
class ModelArguments:
    ...
    tokenizer_name_or_path: Optional[str] = field(default=None, metadata={"help": "Path to the LLM or MLLM."})
    ...
    pretrain_llm: Optional[str] = field(default=None)
    ...
    patch_size: tuple = field(default=(8, 8, 8))
...
class DataArguments:
    ...
    # caption data
    cap_data_path: str = field(default="/raid/export/wqruan/mri_data/tao_train_llm.json", metadata={"help": "Path to caption data."})
...
def main():
    ...
    for param in f1radgraph.parameters():
    ...
    # print(reward_list)
    # exit()
    # print(f1radgraph.radgraph.model._embedder.token_embedder_bert._matched_embedder.transformer_model.embeddings.LayerNorm.bias.device)
    # print(f1radgraph.radgraph.model._embedder.token_embedder_bert._matched_embedder.transformer_model.embeddings.LayerNorm.weight.device)
    # exit()
    # print("initialize f1radgraph success!!!!!!!!!!!!!!!")
    ...
    if model_args.vision_tower is not None:
        if 'llama' in model_args.model_type:
            ...
            #     print(layer.self_attn.qkv_proj.weight.dtype)
                # if layer.self_attn.qkv_proj.weight.dtype != torch.float16:
                    # layer.self_attn.qkv_proj.weight = layer.self_attn.qkv_proj.weight.to(torch.float16)
                    # print(i)
            # print(model.model.layers[0].self_attn.qkv_proj.weight.dtype)
            # exit()
    ...
    model.config.image_channel = model_args.image_channel
    model.config.seg_token_id = model_args.seg_token_id
    model.config.use_cache = False
    model.radgraph_enable = training_args.radgraph_enable
    # if get_rank() == 0:
    #     print("seg_token_id: ", model_args.seg_token_id)
    #     print("vocab_size: ", model_args.vocab_size)
    #     print(len(tokenizer))
    #     # config = AutoConfig.from_pretrained(model_args.model_name_or_path)
    #     print(model.config.vocab_size)  # 应该与tokenizer长度一致
    # exit()
    # model.tokenizer = tokenizer
    # print(model.tokenizer is tokenizer)
    # exit()
    local_rank = dist.get_rank()
    model.f1radgraph = f1radgraph.to(f"cuda:{local_rank}").eval()
    ...
    model.get_model().config.vision_tower = "swin"
    model.get_model().switch_vision_tower()    
    if model_args.vision_tower is not None:
        model.get_model().initialize_vision_modules(model_args=model_args)
    # exit()
    # if get_rank() == 0:
    #     print(type(model.get_model().get_vision_tower()))
    # exit()
