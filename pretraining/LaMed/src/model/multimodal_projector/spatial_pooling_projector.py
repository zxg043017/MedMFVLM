...
class SpatialPoolingProjector(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pooling_type == 'spatial':
            ...
            # print(x.shape)
            # exit()
            # to_seq = Rearrange("b d p1 p2 p3 -> b (p1 p2 p3) d", b=B, d=x.shape[1], p1=x.shape[2], p2=x.shape[3], p3=x.shape[4])
            # to_seq = Rearrange("b d p1 p2 p3 -> b (p1 p2 p3) d")
