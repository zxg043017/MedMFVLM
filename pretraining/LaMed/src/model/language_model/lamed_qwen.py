...
import numpy as np
from transformers import AutoConfig, AutoModelForCausalLM, Qwen2_5_VLConfig, Qwen2_5_VLModel, Qwen2_5_VLForConditionalGeneration
...
class LamedQwen2_5_VLConfig(Qwen2_5_VLConfig):
    model_type = "lamed_qwen2_5"


class LamedQwen2_5_VLModel(LamedMetaModel, Qwen2_5_VLModel):
    config_class = LamedQwen2_5_VLConfig
    def __init__(self, config: Qwen2_5_VLConfig):
        super(LamedQwen2_5_VLModel, self).__init__(config)


class LamedQwen2_5ForCausalLM(LamedMetaForCausalLM, Qwen2_5_VLForConditionalGeneration):
    config_class = LamedQwen2_5_VLConfig

    def __init__(self, config):
        super(LamedQwen2_5ForCausalLM, self).__init__(config)
        self.model = LamedQwen2_5_VLModel(config)
    ...
    def forward(
        ...
        # radgraph loss
            ...
            #     print(label)
            # exit()
            hyps = self.tokenizer.batch_decode(predicted_token_ids, skip_special_tokens=True)
            ...
            if self.count % 16 == 0 and get_rank() == 0:
        ...
        # output score
            ...
            # input_ids = input_ids.to(dtype=torch.bfloat16)
            ...
            # exit()
            # position_ids = position_ids.to(dtype=torch.bfloat16)
            # past_key_values = past_key_values.to(dtype=torch.bfloat16)
            # inputs_embeds = inputs_embeds.to(dtype=torch.bfloat16)
            # labels = labels.to(dtype=torch.bfloat16)
            # if get_rank == 0:
            # print(attention_mask)
            # exit()
            # TODO: check the shape of data when calulating loss, check the attention mask
            outputs = super().forward(
                input_ids=input_ids,
                attention_mask=attention_mask,
                position_ids=position_ids,
            ...
            if is_initialized():
                if (self.count % 160 == 0 or (self.count - 1) % 160 == 0 or (self.count + 1) % 160 == 0) and get_rank() == 0:
                    logits = outputs.logits
                    predicted_token_ids = logits.argmax(dim=-1)
                    hyps = self.tokenizer.batch_decode(predicted_token_ids, skip_special_tokens=True)
                    labels_cp = labels.clone()
                    labels_cp[labels_cp == -100] = self.tokenizer.pad_token_id
                    refs = self.tokenizer.batch_decode(labels_cp, skip_special_tokens=True)
                    # print(f"text loss: {outputs.loss}, radgraph_loss: {radgraph_loss}")
                    print(f"predict: {hyps}")
                    print(f"label: {refs}")
            self.count += 1
            return outputs
...
AutoConfig.register("lamed_qwen2_5", LamedQwen2_5_VLConfig)
AutoModelForCausalLM.register(LamedQwen2_5_VLConfig, LamedQwen2_5ForCausalLM)