...
from ..SwinUNetrClassification import SwinUNETR_Encoder
...
#             # print("when mask is not None, attn:", attn.dtype)
#             attn = self.softmax(attn)
#             # attn = attn.to(torch.bfloat16)
#             # print("when mask is not None, after softmax, attn:", attn.dtype)
#         else:
#             # print("when mask is None, attn:", attn.dtype)
#             attn = self.softmax(attn)
#             # print("when mask is None, after softmax, attn:", attn.dtype)
            
#         # print("before attn:", attn.dtype)
#         attn = self.attn_drop(attn)
#         # print("attn:", attn.dtype, "v:", v.dtype)
...
# class SwinUNETRClassification(nn.Module):
#     def __init__(self, n_class=3) -> None:    ## change to 3
#         super(SwinUNETRClassification, self).__init__()

#         self.encoder = SwinUNETR_Encoder(img_size=(128,128,128), in_channels=1, out_channels=4, feature_size=48)
...
#         print("Encoder output:", feature.shape)
...
class SwinViT3DTower(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        # self.select_layer = config.vision_select_layer
        # self.select_feature = config.vision_select_feature
        # self.hidden_size = 768
        self.vision_tower = SwinUNETR_Encoder(img_size=config.image_size, in_channels=config.image_channel, out_channels=4, feature_size=48).to(torch.float32)
...
if __name__ == "__main__":
    model = SwinUNETRClassification(n_class=3)
    # pretrain = "G:/YJX_Data/Upload/Emergency_Screening/pretrain/swinunetr.pth"
    pretrain = "/share/home/jiangmeirui/data/upload/weights/supervised_suprem_swinunetr_2100.pth"
    # weights = torch.load(pretrain, map_location='cpu')['net']
    ...
    # pred = torch.stack([pred1, pred2, pred3])
    # print(pred.shape)
    # result, _ = torch.max(pred, dim=0)
