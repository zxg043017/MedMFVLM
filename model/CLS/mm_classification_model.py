...
class Vision_Language_Merge_Branch(nn.Module):
    def __init__(self, out_channels=3, text_prompt=True, encoding="word_embedding") -> None:  ## change to 3
...
class Foundation_Model_Classification(nn.Module):
    def __init__(self, n_class = 7, text_prompt = True, VL_CrossAttention = True, fusion_module="Cross_Attention", text_encoding="word_embedding") -> None:
        super(Foundation_Model_Classification, self).__init__()
        self.text_prompt = text_prompt
        self.text_encoding = text_encoding
        self.VL_CrossAttention = VL_CrossAttention
        self.VLM_branch = Vision_Language_Merge_Branch(n_class, text_prompt, encoding=text_encoding)
        self.DVF_branch = Deep_Vision_Feature_Model(model_depth=50, n_classes=256, input_W=96, input_H=96, input_D=96)
        ...
        if fusion_module=="DoubleAttention":
            ...
            self.multimoal_fusion_module = MultimodalFusionAttention(channels=14)
            ...
            self.seg_out_fusion_module = FeatureFusionConcat(in_channels=28, out_channels=14)
    ...
    def forward(self, x1, x2):
        ...
        vision_feature_fusion = self.multimoal_fusion_module(DVF_x1, DVF_x2)
