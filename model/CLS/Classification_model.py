...
class UnetClassification_DAS(nn.Module):
    def __init__(self, n_class=4) -> None:
        super(UnetClassification_DAS, self).__init__()
...
class UnetClassification(nn.Module):
    def __init__(self, n_class=4) -> None:
        ...
        self.cls_head = nn.Sequential(
            # nn.Linear(in_features=512, out_features=128),
            # nn.ReLU(),
            nn.Linear(in_features=640, out_features=n_class)
        )
    ...
    # def forward(self, liver, spleen, left_kidney, right_kidney):
        liver_feature = self.conv3d(liver)
        liver_feature = self.encoder(liver_feature)
        liver_feature = self.GAP(liver_feature)

        # txt_img_fuse = torch.cat([liver_feature, txt_feat], 1)

        cls_pred = self.cls_head(txt_img_fuse)
        return cls_pred
        ...
        #
        # all_feature = torch.concat((liver_feature, spleen_feature, left_feature, right_feature),
        #                            dim=1)  # fusion the model
